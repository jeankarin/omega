...
def main():
    ...
    if (os.path.exists("~/DevLocal/git/omega/Docker/Temporal/mac/files/numeros.txt")):

        # Conseguimos el último ID del fichero si existe o creamos el fichero si no existe
        ultimoID_temp = lectura_fichero.lecturaFichero('~/DevLocal/git/omega/Docker/Temporal/mac/files/lastregistry.txt')
        #print(type(ultimoID_temp))
        ultimoID = ultimoID_temp[0][0][1::]

        # Leemos el fichero numeros.txt
        numeros = lectura_fichero.lecturaFichero('~/DevLocal/git/omega/Docker/Temporal/mac/files/numeros.txt')
        ...
        if error > 0:
            ...
            if (result1 == 0) and (result2 == 0):
                ...
                os.system("rm ~/DevLocal/git/omega/Docker/Temporal/mac/files/numeros.txt")
